...
async function obtenerReservasHoy(req = request, res = response) { //ARREGLADO
    ...
    let var2 = hoy1.setDate(hoy1.getDate());
    let arrayReservas = [];
    const reservas = await getAllReservas();
    if (reservas.length > 0) {
        for (const reserva of reservas) {
            let date = new Date(reserva.RESERVATIONDATE)
            let var1 = date.setDate(date.getDate());
            if (var2 == var1) {
            ...
            if (var2 > var1) {
                if(reserva.IDSTATE!=3){
                    const reservaUpdate = {
                        "IDRESERVA": reserva.IDRESERVA,
                        "IDSTATE": 3
                    }
                    await updateReserva(reservaUpdate)
                }
            }
        }
        res.status(200).json(responseJson(200, "success", arrayReservas))
